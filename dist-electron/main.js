...
async function checkForUpdates(apiUrl, apiKey) {
  try {
    console.log("Verificando actualizaciones disponibles...");
    if (!apiUrl || !apiKey) {
      console.log("No hay configuración de API para verificar actualizaciones");
      return { available: false, message: "Sin configuración de API" };
    }
    ...
    if (response.data && response.data.latest_version) {
      ...
      return {
        ...
        message: response.data.message || "Nueva versión disponible",
        changeLog: response.data.change_log || []
      };
    }
    return { available: false, currentVersion, message: "Sistema actualizado" };
  } catch (error) {
...
function setupUpdateHandlers() {
  ipcMain.handle("check-updates", async (_, { apiUrl, apiKey }) => {
    try {
      const updateInfo = await checkForUpdates(apiUrl, apiKey);
      return updateInfo;
    } catch (error) {
      console.error("Error en el manejador de actualizaciones:", error);
      return {
        available: false,
        error: error.message
      };
